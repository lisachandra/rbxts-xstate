...
export type GetParameterizedParams<T extends ParameterizedObject | undefined> = T extends any
...
export type AnyObject = { [K: PropertyKey]: unknown };
export type InferAnyObject<K extends PropertyKey, V> = AnyObject & {
	[k in K]: V;
};
...
export type Merge<M, N> = Omit<M, keyof N extends keyof M ? keyof N : never> & N;
...
export type ConditionalRequired<T, Condition extends boolean> = Condition extends true
	? Required<T>
	: T;
...
export type Actions<
	...
	TEmitted extends EventObject,
> = SingleOrArray<
	Action<TContext, TExpressionEvent, TEvent, TParams, TActor, TAction, TGuard, TDelay, TEmitted>
>;
...
> {
	...
	toJSON(): Omit<
		InvokeDefinition<TContext, TEvent, TActor, TAction, TGuard, TDelay, TEmitted, TMeta>,
			...
			| Compute<
					{
						systemId?: string;
						/**
						 * The source of the machine to be invoked, or the
						 * machine itself.
						 */
						src: TSrc;

						/**
						 * The unique identifier for the invoked machine. If not
						 * specified, this will be the machine's own `id`, or
						 * the URL (from `src`).
						 */
						...
						/**
						 * The transition to take upon the invoked child machine
						 * reaching its final top-level state.
						 */
						onDone?:
							| string
							| SingleOrArray<
...
> =
	IsLiteralString<TActor["src"]> extends true
		...
		: {
				/**
				 * The unique identifier for the invoked machine. If not
				 * specified, this will be the machine's own `id`, or the URL
				 * (from `src`).
				 */
				id?: string;
				...
				systemId?: string;
				/**
				 * The source of the machine to be invoked, or the machine
				 * itself.
				 */
				...
				onDone?:
					...
					| SingleOrArray<
					  ...
					  >;
				/**
				 * The transition to take upon the invoked child machine sending
				 * an error event.
				 */
				onError?:
					| string
...
> {
	...
	/**
	 * Indicates whether the state node is a history state node, and what type
	 * of history: shallow, deep, true (shallow), false (none), undefined
	 * (none)
	 */
	...
	invoke?: SingleOrArray<
		InvokeConfig<TContext, TEvent, TActor, TAction, TGuard, TDelay, TEmitted, TMeta>
	>;
	 ...
	 * transition(s). The delayed transitions are taken after the specified
	 * delay in an interpreter.
	 */
	 ...
	 * final state node.
	 *
	 * The output data will be evaluated with the current `context` and placed
	 * on the `.data` property of the event.
	 */
...
export interface StateMachineDefinition<TContext extends MachineContext, TEvent extends EventObject>
...
export interface AtomicStateNodeConfig<TContext extends MachineContext, TEvent extends EventObject>
...
export interface HistoryStateNodeConfig<TContext extends MachineContext, TEvent extends EventObject>
	extends AtomicStateNodeConfig<TContext, TEvent> {
	history: "shallow" | "deep" | true;
	target: string | undefined;
}

export type SimpleOrStateNodeConfig<TContext extends MachineContext, TEvent extends EventObject> =
...
export interface ErrorActorEvent<TErrorData = unknown, TId extends string = string>
	extends EventObject {
...
export interface SnapshotEvent<TSnapshot extends Snapshot<unknown> = Snapshot<unknown>>
	extends EventObject {
...
export interface TransitionDefinition<TContext extends MachineContext, TEvent extends EventObject>
	extends Omit<
		TransitionConfig<
			TContext,
			TEvent,
...
export interface ActorOptions<TLogic extends AnyActorLogic> {
	 ...
	 *
	 * @remarks
	 * You can create your own “clock”. The clock interface is an object with
	 ...
	 *
	 * By default, the native `setTimeout` and `clearTimeout` functions are
	 * used.
	 *
	 ...
	 *
	 * @remarks
	 * If the state is compatible with the actor logic, when the actor is
	 * started it will be at that persisted state. Actions from machine actors
	 * will not be re-executed, because they are assumed to have been already
	 * executed. However, invocations will be restarted, and spawned actors will
	 * be restored recursively.
	 *
...
> extends Subscribable<TSnapshot>,
	...
	on<TType extends TEmitted["type"] | "*">(
		type: TType,
		handler: (emitted: TEmitted & (TType extends "*" ? unknown : { type: TType })) => void,
	): Subscription;
}
...
> {
	...
	/**
	 * Obtains the internal state of the actor in a representation which can be
	 * be persisted. The persisted state can be restored by `restoreSnapshot`.
	 *
	 * @param snapshot - The current state.
...
export type GetConcreteByKey<T, TKey extends keyof T, TValue extends T[TKey]> = T &
	Record<TKey, TValue>;

type _GroupStateKeys<T extends StateSchema, S extends keyof T["states"]> = S extends any
