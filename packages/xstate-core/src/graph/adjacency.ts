import { ActorScope, ActorLogic, ActorSystem, EventObject, Snapshot } from "../index";
import {
	SerializedEvent,
	SerializedSnapshot,
	TraversalOptions,
...
>(
	...
	while (queue.size()) {
		...
		const serializedState = serializeState(state, event, prevState) as SerializedSnapshot;
		...
		for (const nextEvent of events) {
			const nextSnapshot = logic.transition(state, nextEvent, actorScope);

			adj[serializedState].transitions[serializeEvent(nextEvent) as SerializedEvent] = {
				event: nextEvent,
				state: nextSnapshot,
			};
			queue.push({
