...
import { SerializedEvent, SerializedSnapshot, StatePath, Steps, TraversalOptions } from "./types";
import {
...
function isMachine(value: unknown): value is AnyStateMachine {
	return !!value && typeIs(value, "table") && "__xstatenode" in value;
}
...
>(
	...
	const resolvedOptions = resolveTraversalOptions(
		...
		(isMachine(logic)
			...
			: createDefaultLogicOptions()) as TraversalOptions<TSnapshot, TEvent, TInput>,
	);
	const actorScope = createMockActorScope() as ActorScope<TSnapshot, TEvent, TSystem>;
	const fromState =
	...
	for (const event of events) {
		...
		const nextStateSerial = serializeState(nextState, event, prevState) as SerializedSnapshot;
