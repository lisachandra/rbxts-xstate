import { AnyActorLogic, EventFromLogic, InputFrom } from "../index";
import {
	SerializedEvent,
	SerializedSnapshot,
	StatePath,
...
export function getSimplePaths<TLogic extends AnyActorLogic>(
	...
	const fromState = ((resolvedOptions.fromState as unknown) ??
		(logic.getInitialSnapshot(actorScope, options?.input) as unknown)) as never;
	...
	function util(fromStateSerial: SerializedSnapshot, toStateSerial: SerializedSnapshot) {
	...
	const simplePaths = Array.flatMap(Object.values(pathMap), p => p.paths);
