...
export function getShortestPaths<TLogic extends AnyActorLogic>(
	...
	const fromState = ((resolvedOptions.fromState as unknown) ??
		(logic.getInitialSnapshot(createMockActorScope(), options?.input) as unknown)) as never;
	...
	weightMap.forEach(({ weight, state: fromState, event: fromEvent }, stateSerial) => {
		const state = stateMap.get(stateSerial)!;
		const steps = !fromState
			? []
			: Array.concat(statePlanMap[fromState].paths[0].steps, {
