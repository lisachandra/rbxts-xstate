...
> {
  ...
  addEventListener<T extends string & keyof TEventMap>(
    ...
    const duplicate = signalData.listeners.find(
      (listener) =>
        listener.callback === callback && isCapture(listener) === capture,
    );
    ...
    const connection = signalData.signal.Connect((event: Event) => {
      if (passive && event.preventDefault) {
        ...
        event.preventDefault = () => {
          throw new Error(
            "preventDefault() called inside passive event listener",
  ...
  removeEventListener<T extends string & keyof TEventMap>(
    ...
    const listenerMap = _(this);
    const { callback, kind: kind } = normalizeOptions(
      type0,
      callback0,
      options0,
    );
...
export namespace EventTarget {
  ...
  > = TMode extends "standard"
    ? EventListener<TEventTarget, Event> | undefined
    : never;

  /** The event type in standard mode. Otherwise, never. */
  export type FallbackEvent<TMode extends "standard" | "strict"> =
    TMode extends "standard" ? Event : never;

  /**
   * Check if given event map is valid. It's valid if the keys of the event map
   * are narrower than `string`.
   */
