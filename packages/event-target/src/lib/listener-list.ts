...
export interface ListenerList {
	...
	/**
	 * `true` if the `dispatchEvent` method is traversing the current
	 * `listeners` array.
	 */
	cow: boolean;
	/** The listeners. This is writable for copy-on-write. */
	listeners: Listener[];
}

/**
 * Find the index of given listener. This returns `-1` if not found.
 *
...
/**
 * Add the given listener. Does copy-on-write if needed.
 *
 * @param list The listener list.
 ...
 */
export function removeListenerAt(list: ListenerList, index: number, disableCow = false): boolean {
