...
export async function testQueryHelpers() {
	...
	const createIdRelatedErrorHandler =
		...
		<T>(container: Element | null, ...args: T[]) => {
			...
			if (key && value) {
				return errorMessage.replace("[key]", String(key)).replace("[value]", String(value));
			}
...
export function testBoundFunctions() {
	const boundfunctions = {} as BoundFunctions<{
		...
		customQueryTwo: (container: HTMLElement, text: string, text2: string) => HTMLElement;
...
export async function testByRole() {
	...
	console.assert(
		(await findByRole(element, "button", { hidden: true }, { timeout: 10 })) !== null,
	);

	console.assert(
		queryAllByRole(document.body, "progressbar", { queryFallbacks: true }).length === 1,
	);
