import {
	...
	prettyFormat,
	Config as ConfigDTL,
} from "@rbxts/dom-testing-library";
import { act as reactAct } from "@rbxts/react";
...
> {
	...
	/**
	 * Only works if used with React 18. Set to `true` if you want to force
	 * synchronous `ReactDOM.render`. Otherwise `render` will default to
	 * concurrent React if available.
	 */
	legacyRoot?: boolean | undefined;
	/**
	 * Only supported in React 19. Callback called when React catches an error
	 * in an Error Boundary. Called with the error caught by the Error Boundary,
	 * and an `errorInfo` object containing the `componentStack`.
	 *
	...
	onCaughtError?: ReactDOMClient.RootOptions extends {
		...
		: never;
	/**
	 * Callback called when React automatically recovers from errors. Called
	 * with an error React throws, and an `errorInfo` object containing the
	 * `componentStack`. Some recoverable errors may include the original error
	 * cause as `error.cause`.
	 *
	 * @see {@link https://react.dev/reference/react-dom/client/createRoot#parameters createRoot#options}
	 */
	...
	/**
	 * When enabled, <StrictMode> is rendered around the inner element. If
	 * defined, overrides the value of `reactStrictMode` set in `configure`.
	 */
	reactStrictMode?: boolean;
}
...
export function render(ui: React.ReactNode, options?: Omit<RenderOptions, "queries">): RenderResult;
...
export interface RenderHookResult<Result, Props> {
	...
	/**
	 * This is a stable reference to the latest value returned by your
	 * renderHook callback
	 */
	result: {
...
/**
 * Simply calls React.act(cb) If that's not available (older version of react)
 * then it simply calls the deprecated version which is ReactTestUtils.act(cb)
 */
