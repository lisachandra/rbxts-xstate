...
test("does not work after it resolves", function()
	jest.useFakeTimers()
		...
		unstable_advanceTimersWrapper = function(callback)
			...
			local ok, result = pcall(function(): any
				...
				if typeof(if typeof(result) == "table" then result.andThen else nil) == "function" then
					...
					-- ROBLOX deviation START: we don't have a broadly usable
					-- concept of `thennables` that can be awaited, so instead
