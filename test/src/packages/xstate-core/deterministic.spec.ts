...
describe("deterministic machine", () => {
	...
	describe("machine transitions", () => {
		it("should properly transition states based on event-like object", () => {
			expect(
				transition(lightMachine, lightMachine.resolveState({ value: "green" }), {
					type: "TIMER",
				})[0].value,
			).toEqual("yellow");
		});
		...
		it("should use the machine.initialState when an undefined state is given", () => {
			...
			expect(transition(lightMachine, init, { type: "TIMER" })[0].value).toEqual("yellow");
		});
	...
	describe("machine transition with nested states", () => {
		it("should properly transition a nested state", () => {
			expect(
				transition(lightMachine, lightMachine.resolveState({ value: { red: "walk" } }), {
					type: "PED_COUNTDOWN",
				})[0].value,
			).toEqual({ red: "wait" });
		});
		...
		it("should bubble up events that nested states cannot handle", () => {
			expect(
				transition(lightMachine, lightMachine.resolveState({ value: { red: "stop" } }), {
					type: "TIMER",
				})[0].value,
			).toEqual("green");
		});
		...
		it("should transition to the deepest initial state", () => {
			expect(
				transition(lightMachine, lightMachine.resolveState({ value: "yellow" }), {
					type: "TIMER",
				})[0].value,
			).toEqual({
				red: "walk",
	...
	describe("state key names", () => {
		...
		it("should work with substate nodes that have the same key", () => {
			...
			expect(transition(machine, init, { type: "NEXT" })[0].value).toEqual("test");
		});
	});
