...
describe("multiple", () => {
		...
		states: {
			simple: {
				on: {
					...
					BROKEN_DIFFERENT_REGIONS: [{ target: ["para.A.C", "para.K.M", "other"] }],
	...
	describe("transitions to parallel states", () => {
		...
		it.skip("should reject targets inside and outside a region", () => {
			const actorRef = createActor(machine).start();
			expect(() => actorRef.send({ type: "BROKEN_DIFFERENT_REGIONS" })).toThrow();
		});

		it.skip("should reject two targets in different regions", () => {
			const actorRef = createActor(machine).start();
			expect(() => actorRef.send({ type: "BROKEN_DIFFERENT_REGIONS_2" })).toThrow();
		});

		it.skip("should reject two targets in different regions at different levels", () => {
			const actorRef = createActor(machine).start();
			expect(() => actorRef.send({ type: "BROKEN_DIFFERENT_REGIONS_3" })).toThrow();
		});
		...
		it.skip("should reject two deep targets in different regions at different levels", () => {
			const actorRef = createActor(machine).start();
			expect(() => actorRef.send({ type: "BROKEN_DIFFERENT_REGIONS_4" })).toThrow();
		});
	});
});
