...
describe("State", () => {
	...
	describe(".can", () => {
		it("should return true for a simple event that results in a transition to a different state", () => {
			...
			expect(createActor(machine).getSnapshot().can({ type: "NEXT" })).toBe(true);
		});
		...
		it("should return false for a forbidden transition", () => {
			...
			expect(createActor(machine).getSnapshot().can({ type: "EV" })).toBe(false);
		});
		...
		it("should return false for an unknown event", () => {
			...
			expect(createActor(machine).getSnapshot().can({ type: "UNKNOWN" })).toBe(false);
		});
		...
		it("should return true when non-first parallel region changes value", () => {
			...
			expect(createActor(machine).getSnapshot().can({ type: "EVENT" })).toBeTruthy();
		});
	...
	describe(".status", () => {
		it("should be 'stopped' after a running actor gets stopped", () => {
			const snapshot = createActor(createMachine({})).start().stop().getSnapshot();
