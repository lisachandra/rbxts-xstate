...
import { AnyMachineSnapshot, AnyStateMachine, createActor } from "../src/index.ts";
...
async function runTestToCompletion(machine: AnyStateMachine, test: SCIONTest): Promise<void> {
...
describe("scxml", () => {
	...
	testGroupKeys.forEach(testGroupName => {
		...
		testNames.forEach(testName => {
			...
			const scxmlSource =
				overrides[testGroupName] && overrides[testGroupName].indexOf(testName) !== -1
					? `./fixtures/scxml/${testGroupName}/${testName}.scxml`
					: `${TEST_FRAMEWORK}/test/${testGroupName}/${testName}.scxml`;
			const scxmlDefinition = fs.readFileSync(path.resolve(__dirname, scxmlSource), {
				encoding: "utf-8",
			});
