...
describe("invalid or resolved states", () => {
	...
	it("should resolve transitions from empty states", () => {
		...
		expect(
			getNextSnapshot(machine, machine.resolveState({ value: { A: {}, B: {} } }), {
				type: "E",
			}).value,
		).toEqual({
			A: "A1",
	...
	it("should allow transitioning from valid states", () => {
		...
		getNextSnapshot(machine, machine.resolveState({ value: { A: "A1", B: "B1" } }), {
			type: "E",
		});
	});
	...
	it("should reject transitioning from bad state configs", () => {
		...
		expect(() =>
			getNextSnapshot(machine, machine.resolveState({ value: { A: "A3", B: "B3" } }), {
				type: "E",
			}),
		).toThrow();
	});
	...
	it("should resolve transitioning from partially valid states", () => {
		...
		expect(
			getNextSnapshot(machine, machine.resolveState({ value: { A: "A1", B: {} } }), {
				type: "E",
			}).value,
		).toEqual({
			A: "A1",
