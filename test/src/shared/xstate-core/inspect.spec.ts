...
function simplifyEvents(
	...
	return inspectionEvents.filter(filter ?? (() => true)).map(inspectionEvent => {
...
describe("inspect", () => {
	...
	it("should inspect microsteps for normal transitions", () => {
		...
		expect(simplifyEvents(events as any[])).toEqual([
			{
				actorId: "x:6",
				type: "@xstate.actor",
			},
	...
	it("should inspect actions", () => {
		...
		expect(simplifyEvents(events, ev => ev.type === "@xstate.action")).toEqual([
			{
				action: {
					params: undefined,
					type: "enter1",
