...
describe("emitted", () => {
	it("emitted type should be represented in actor.on(…)", () => {
		const m = setup({
			types: {
				emitted: {} as { type: "onClick"; x: number; y: number } | { type: "onChange" },
			},
		}).createMachine({});
...
describe("events", () => {
	...
	it("event type should be inferable from a simple state machine type", () => {
		...
		function acceptMachine<TContext extends {}, TEvent extends { type: string }>(
	...
	it("should infer inline function parameters when for a wildcard transition", () => {
		createMachine({
			...
			on: {
				"*": {
					actions: ({ event }) => {
						((_accept: "EVENT_WITH_FLAG" | "EVENT_WITHOUT_FLAG") => {})(event.type);
						// @ts-expect-error
						((_accept: "is not any") => {})(event);
					},
				},
...
describe("spawnChild action", () => {
	...
	it(`should reject static wrong input`, () => {
		const child = fromPromise(({}: { input: number }) => Promise.resolve("foo"));
	...
	it(`should allow static input that is a subtype of the expected one`, () => {
		const child = fromPromise(({}: { input: number | string }) => Promise.resolve("foo"));
	...
	it(`should reject a valid input of a different provided actor`, () => {
		...
		const child2 = fromPromise(({}: { input: string }) => Promise.resolve("foo"));
	...
	it(`should not require input when it's optional`, () => {
		const child = fromPromise(({}: { input: number | undefined }) => Promise.resolve(100));
...
describe("state.children without setup", () => {
	...
	it("should have an index signature on the available snapshot when child IDs were configured only for some actors", () => {
		...
		someActor satisfies ActorRefFrom<typeof child1> | ActorRefFrom<typeof child2> | undefined;
	});
});
...
describe("createActor", () => {
	...
	it(`should not require input when it's optional`, () => {
		const logic = fromPromise(({}: { input: number | undefined }) => Promise.resolve(100));
