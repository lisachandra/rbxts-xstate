...
import { assign, createMachine } from "@rbxts/xstate";
import { joinPaths } from "@rbxts/xstate/out/graph";
import { getShortestPaths } from "@rbxts/xstate/out/graph/shortestPaths";
...
describe("getShortestPaths", () => {
	...
	it("finds the shortest paths from a state to another state", () => {
		...
		const paths = Array.flatMap(pathsToB, path => {
		...
		expect(paths[0].steps.map(s => s.event.type)).toEqual([
			"xstate.init",
			"TO_B",
			"NEXT_B_TO_X",
			"NEXT_X_TO_Y",
	...
	it("handles event cases", () => {
		const machine = createMachine({
			...
			on: {
				"todo.add": {
					actions: assign({
						todos: ({ context, event }) => {
							return Array.concat(context.todos, event.todo);
						},
					}),
				},
			},
		...
		const shortestPaths = getShortestPaths(machine, {
			...
			stopWhen: state => state.context.todos.size() >= 3,
		});
