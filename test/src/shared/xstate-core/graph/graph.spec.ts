...
import { createMockActorScope } from "@rbxts/xstate/out/graph/actorScope";
import {
	StatePath,
	getPathsFromEvents,
	getShortestPaths,
		...
		steps: path.steps.map(step => ({
			state: isMachineSnapshot(step.state)
				...
				: "context" in step.state
					? step.state.context
					: step.state,
			eventType: (step.event as AnyObject).type as any,
		})),
	};
}
...
describe("@xstate/graph", () => {
	...
	describe("getShortestPaths()", () => {
		...
		it.skip("should not throw when a condition is present", () => {
			expect(() => getShortestPaths(condMachine)).never.toThrow();
		});
	...
	describe("getSimplePaths()", () => {
		...
		it("should return a mapping of simple paths to all states (parallel)", () => {
			...
			expect(paths.map(p => p.state.value)).toEqual([
				{
					a: "a1",
					b: "b1",
				},
		...
		it("should return multiple paths for equivalent transitions", () => {
			...
			expect(paths.map(p => p.state.value)).toEqual(["a", "b", "b"]);
		...
		it("should return a single-length path for the initial state", () => {
			expect(
				getSimplePaths(lightMachine).find(p =>
					p.state.matches(lightMachine.getInitialSnapshot(createMockActorScope()).value),
				),
			).toBeDefined();
			expect(
				getSimplePaths(lightMachine).find(p =>
			...
			expect(
				getSimplePaths(equivMachine).find(p =>
					p.state.matches(equivMachine.getInitialSnapshot(createMockActorScope()).value),
				)!,
			).toBeDefined();
			expect(
				getSimplePaths(equivMachine).find(p =>
		...
		it("should return value-based paths", () => {
			...
			expect(paths.map(p => p.state.value)).toEqual(["start", "start", "start", "finish"]);
	...
	describe("getPathFromEvents()", () => {
		...
		it.skip("should throw when an invalid event sequence is provided", () => {
			expect(() =>
				getPathsFromEvents(lightMachine, [{ type: "TIMER" }, { type: "INVALID_EVENT" }]),
			).toThrow();
		});
...
describe("filtering", () => {
	it("should not traverse past filtered states", () => {
		...
		expect(shortestPaths.map(p => p.state.context)).toEqual([
			{
				count: 0,
			},
			{
...
it("should provide previous state for serializeState()", () => {
	...
	// 4 (a -> b -> c -> a)
	expect(
		shortestPaths.filter(path => path.state.matches("a")).map(path => path.steps.size()),
	).toEqual([1, 4]);
});

it.each([getShortestPaths, getSimplePaths])("from-state can be specified", pathGetter => {
	const machine = createMachine({
		initial: "a",
		states: {
			a: {
	...
	expect(paths.find(path => path.state.matches("b") && path.steps.size() === 1)).toBeTruthy();

	// Instead of starting at state 'a', it should take > 0 steps to reach 'a'
	expect(paths.find(path => path.state.matches("a") && path.steps.size() > 0)).toBeTruthy();
});
...
describe("joinPaths()", () => {
	it("should join two paths", () => {
		...
		expect(pathToBAndC.steps.map(step => step.event.type)).toEqual([
			"xstate.init",
			"NEXT",
			"TO_C",
		]);
	...
	it("should not join two paths with mismatched source/target states", () => {
		...
		}).toThrowError(RegExp("Paths cannot be joined"));
	});
});
