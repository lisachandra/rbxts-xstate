...
import { assign, createMachine, setup } from "@rbxts/xstate";
...
describe("state tests", () => {
	...
	it("should test nested states", async () => {
		const testedStateValues: defined[] = [];
		...
		expect(testedStateValues).toEqual(["a", "b", "b.b1"]);
	});
	...
	it("should test with input", () => {
		const machine = setup({
			...
			states: {
				checking: {
					always: [
						{ guard: x => x.context.name.size() > 3, target: "longName" },
						{ target: "shortName" },
					],
				},
				longName: {},
